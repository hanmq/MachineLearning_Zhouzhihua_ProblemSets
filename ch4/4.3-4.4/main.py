...
class DecisionTree(object):
    ...
    def fit(self, X_train, y_train, X_val=None, y_val=None):
        ...
        生成决策树
        -------
        :param X:  只支持DataFrame类型数据，因为DataFrame中已有列名，省去一个列名的参数。所以不支持np.array等其他数据类型
        :param y:
        :return:
        '''

        assert (self.pruning is None and X_val is None and y_val is None) or (
                    self.pruning is not None and X_val is not None and y_val is not None)

        self.columns = list(X_train.columns)  # 包括原数据的列名
        self.tree_ = self.generate_tree(X_train, y_train)

        if self.pruning == 'pre_pruning':
            pruning.pre_pruning(X_train, y, X_val, y_val, self.tree_)
        elif self.pruning == 'post_pruning':
            pruning.post_pruning(X_train, y, X_val, y_val, self.tree_)
    ...
    def info_gainRatio(self, feature, y, entD, is_continuous=False):
        ...
        if is_continuous:
            ...
            # 本代码中，对连续值的特征进行了去重，在网上查了很多资料博客，没有这一步，其实最后结果都是一样的。去重之后，可以减少计算量。
            # 另外关于 信息增益修正中，N的值，网上有些资料认为是“可能分裂点的个数”，也有的是“当前特征的取值个数”，这里采用“当前特征的取值个数”。
